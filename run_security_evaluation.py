...
def main():
    ...
    judge_model = cfg.resolve_judge()
    harness.banner(
        "Launching Security Evaluation Engine (Hybrid Mode)",
        cfg,
        target_project=target_repo,
        local_judge=judge_model,
    )
    ...
    if not project_map.get("serializers") and not project_map.get("views"):
        print(
            "   Warning: No Django views or serializers detected. "
            "Attack surface may be incomplete."
        )
    ...
    # 2b. RAG retrieval: surface actual method bodies (authorize_*, get_queryset)
    # so the model can verify authorization rather than guessing. This directly
    # addresses the parser's "cannot trace into service functions" limitation.
    print("Step 2b: Retrieving authorization-relevant code via BM25...")
    rag_query = (
        "authorization permission check get_queryset authorize permission_classes "
        "has_object_permission object-level scoping tenant filter user ownership"
    )
    rag_block = retrieve_relevant_code(rag_query, target_repo, top_k=8, max_chars=12000)
    if rag_block:
        print(f"   [Done] Retrieved {len(rag_block)} chars of relevant code")
    else:
        print("   [Skipped] No retrievable code (indexing unavailable)")

    # The MCP context manager guarantees orch.stop() runs even if the LLM
    ...
    with harness.mcp_context(mcp_config_path, target_repo) as orch:
        ...
        trimmed_map = trim_for_security(project_map)
        project_map_json = json.dumps(trimmed_map, default=str, separators=(",", ":"))
        ...
        parser_limitations = (
            ...
            f"{mcp_block}\n"
            "Retrieved Authorization Implementations (from codebase, ground truth):\n"
            f"{rag_block}\n"
            "Penetration Test Framing:\n"
            "Treat the analysis as a manual penetration test. Think about how an attacker would chain weaknesses together into a realistic exploit path. Each step must trace back to visible evidence. Do NOT invent endpoints, data flows, or trust boundaries to satisfy a narrative. An attack path predicated on a hallucinated queryset or class_attributes value is invalid — verify every attribute against the specific view's entry. The 'Retrieved Authorization Implementations' block above contains ACTUAL method bodies — use it to confirm whether get_queryset/authorize_* functions enforce scoping, instead of assuming auth is absent."
        )

        # Shared context is injected into Pass 1 ONLY; the runner threads
        # assistant turns so Pass 2 sees it via conversation history. This
        # avoids re-sending the full project map on every pass.
        ...
        # Must be self-contained (history resets on fallback), so include everything.
        fallback_prompt = (
            f"{shared_context_block}\n\n"
            "Review Instructions:\n"
            "Conduct a thorough security audit of the codebase represented above. "
            "Apply the severity calibration rules above before making any finding.\n"
            "\n"
        ...
        pass1 = (
            ...
            "Review the extracted codebase topography (provided above) for security vulnerabilities.\n"
            "\n"
        ...
        pass2 = (
            ...
            "Review your findings from Pass 1 against the topography and retrieved code provided above.\n"
            "\n"
        ...
        passes = [fallback_prompt] if not cfg.is_cloud else [pass1, pass2]

        # 4. Execute Reasoning Pass
        print(f"Step 3: Processing security analysis via [{cfg.reasoning_model}]...")
        pass_start = time.time()
        _progress_indicator()
        try:
            final_analysis, model_used, _history = harness.run_multipass(
                system_prompt=system_agent_prompt,
                passes=passes,
                shared_context=(shared_context_block if len(passes) > 1 else ""),
                role="audit",
                num_ctx=49152,
        ...
        if not final_analysis:
            print("   [ERROR] Security analysis produced no output.")
            return

        print(
            f"   [Done] Security analysis via {model_used} "
            f"in {time.time() - pass_start:.2f}s"
        )

        # 5. Verify claims against codebase (same guardrails as code review)
        print("Step 3b: Verifying claims against codebase...")
        verify_start = time.time()
        known_models = {
            m.get("class") or m.get("name") or "" for m in project_map.get("models", [])
        }
        known_models.discard("")
        claims = extract_claims(final_analysis, known_models=known_models)
        if claims:
            verification_report = verify_claims(
                claims, project_map, target_repo=target_repo
            )
            final_analysis = annotate_review(final_analysis, verification_report)
            print(f"   [Done] Claim verification in {time.time() - verify_start:.2f}s")
            print(f"   {verification_report.to_summary()}")
        else:
            print("   [Skipped] No extractable claims found")

        # 6. Evaluate analysis quality + archive (judge family-separated centrally)
        print(f"Step 4: Evaluating analysis quality via Judge [{judge_model}]...")
        ...
        scores = harness.grade_and_archive(
            ...
            judge_override=judge_model,
            report_path="reports/automated_security_review.md",
        )
        ...
        if orch:
            orch.remember(
                ...
                "Security evaluation completed. Report: reports/automated_security_review.md",
                tags=["evaluation", "security", "complete"],
            )

    total_duration = time.time() - start_time
    print("\nReport saved to: reports/automated_security_review.md")
