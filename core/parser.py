...
TRUSTED_AUTH_FUNCTIONS = {
    "authorize_app_user",
    "authorize_benefactor",
    "authorize_creator",
    "authorize_superuser",
    "authorize_staff_or_superuser",
    "authorize_benefactor_or_creator",
    "authorize_benefactor_scope",
    "authorize_content_owner_or_staff",
    "authorize_benefactor_owner_or_staff",
}
...
class DjangoTopographer:
    ...
    def scan_project(self) -> dict:
        ...
        topology: dict[str, Any] = {"serializers": [], "views": [], "models": []}
        for app_dir in self._find_app_dirs():
    ...
    def _resolve_meta_fields(self, value_node, class_name: str, file_classes: dict):
        ...
        if (
            ...
            and value_node.attr == "fields"
            and isinstance(value_node.value, ast.Attribute)
            and value_node.value.attr == "Meta"
        ):
    ...
    def _find_meta_fields(
        self, class_node: ast.ClassDef, class_name: str, file_classes: dict
    ):
                    ...
                    if isinstance(meta_item, ast.Assign):
                        for target in meta_item.targets:
                            if isinstance(target, ast.Name) and target.id == "fields":
                                return self._resolve_meta_fields(
                                    meta_item.value, class_name, file_classes
                                )
        return None
            ...
            for stmt in body:
                if isinstance(stmt, ast.Return) and isinstance(stmt.value, ast.Call):
                    ...
                    if (
                        ...
                        and func.id == "HttpResponseNotAllowed"
                    ):
                        return True, "returns HttpResponseNotAllowed"
                    if (
                        isinstance(func, ast.Attribute)
                        and func.attr == "HttpResponseNotAllowed"
                    ):
                        return True, "returns HttpResponseNotAllowed"
    ...
    def _analyze_permission_class(
        self, class_name: str, imports: dict, current_file: Path
    ) -> dict:
    ...
    def _is_self_scoped(self, func_def: ast.FunctionDef) -> bool:
        ...
        has_catchall = (
            func_def.args.vararg is not None or func_def.args.kwarg is not None
        )
        ...
        for node in ast.walk(func_def):
                ...
                if src_name in ("kwargs", "data", "query_params", "args"):
                    for arg in node.args:
                        if (
                            ...
                            and arg.value.lower() in user_id_keys
                        ):
            ...
            if isinstance(node, ast.Subscript):
                ...
                if isinstance(src, ast.Name):
                    ...
                    src_name = src.attr
                if (
                    src_name in ("kwargs", "data", "query_params", "args")
                    and isinstance(node.slice, ast.Constant)
                    and isinstance(node.slice.value, str)
                    and node.slice.value.lower() in user_id_keys
                ):
                    extracts_user_id = True
        ...
        return (
            has_request_param
            and not extracts_user_id
            and not has_explicit_id_param
            and not has_catchall
        )
        ...
        for item in class_node.body:
            if isinstance(item, ast.FunctionDef):
                ...
                entry = {
                    ...
                    "auth_fully_trusted": bool(
                        auth_calls
                        and all(c in TRUSTED_AUTH_FUNCTIONS for c in auth_calls)
                    ),
                }
                if http:
    ...
    def _parse_class_body_assignments(
        self, class_node: ast.ClassDef, target_names: set[str] | None = None
    ) -> dict:
            ...
            if isinstance(item, ast.Assign):
                for target in item.targets:
                    if isinstance(target, ast.Name) and (
                        target_names is None or target.id in target_names
                    ):
                        attrs[target.id] = self._resolve_ast_value(item.value)
        return attrs

    def _parse_file(self, file_path: Path, topology: dict[str, Any]):
        try:
                            ...
                            for d in child.decorator_list:
                                if isinstance(d, ast.Call) and isinstance(
                                    ...
                                    if d.func.id == "permission_classes" and d.args:
                                        deco_attrs["permission_classes"] = (
                                            self._resolve_ast_value(d.args[0])
                                        )
                                    if d.func.id == "authentication_classes" and d.args:
                                        deco_attrs["authentication_classes"] = (
                                            self._resolve_ast_value(d.args[0])
                                        )
            ...
            for child in tree.body:
                ...
                base_names = {
                    getattr(b, "id", getattr(b, "attr", "")) for b in child.bases
                }

                is_model = bool(base_names & self._model_base_names) or any(
                    isinstance(b, ast.Attribute)
                    and isinstance(b.value, ast.Name)
                    and b.value.id == "models"
                    and b.attr == "Model"
                ...
                if is_model:
                    ...
                    any(k in name for k in ["View", "ViewSet", "APIView"])
                    ...
                    if isinstance(raw_perms, list):
                        for pcls_name in raw_perms:
                            if (
                                isinstance(pcls_name, str)
                                and pcls_name not in DRF_BUILTIN_PERMISSIONS
                            ):
                                analysis = self._analyze_permission_class(
                                    pcls_name, imports, file_path
                                )
                    ...
                    gq_method = next(
                        (m for m in methods if m["name"] == "get_queryset"), None
                    )
            ...
            if isinstance(item, ast.Assign):
                for target in item.targets:
                    if isinstance(target, ast.Name):
                        ...
                        if isinstance(item.value, ast.Call) and hasattr(
    ...
    def _parse_serializer_class(
        self, class_node: ast.ClassDef, file_classes: dict
                        ...
                        if isinstance(item.value, ast.Call):
                            ...
                            for kw in item.value.keywords:
                                if kw.arg in (
                                    ...
                                    field_info[kw.arg] = self._resolve_ast_value(
                                        kw.value
                                    )
                        fields.append(field_info)
                    ...
                    if isinstance(meta_item, ast.Assign):
                        for target in meta_item.targets:
                            if isinstance(target, ast.Name):
                                ...
                                elif target.id == "model" and isinstance(
                                    meta_item.value, ast.Name
                                ):
...
def scan_celery_tasks(target_dir: str) -> list[dict]:
    ...
    if head:
        ...
        if cached is not None:
            ...
            return cached
    results = []
    for _root, rel in _walk_py_files(target_dir):
        try:
            with open(os.path.join(target_dir, rel), encoding="utf-8") as f:
                content = f.read()
        except Exception:
            continue
...
def scan_files_by_keyword(
    target_dir: str, keyword: str, max_lines: int = 80
) -> list[dict]:
    head = get_git_head(target_dir)
    if head:
        key = make_key(
            "parser:scan_files_by_keyword", target_dir, head, keyword, str(max_lines)
        )
        cached = cache_get(key, max_age=86400)
        if cached is not None:
            assert isinstance(cached, list)
...
def scan_files_by_pattern(
    target_dir: str, patterns: list[str], max_lines: int = 80
) -> list[dict]:
    head = get_git_head(target_dir)
    if head:
        patterns_key = ",".join(sorted(patterns))
        key = make_key(
            "parser:scan_files_by_pattern",
            target_dir,
            head,
            patterns_key,
            str(max_lines),
